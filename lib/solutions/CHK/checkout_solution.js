export default class CheckoutSolution {
  ...
  getNewPrice(skuMap) {
    const prices = { X: 17, Y: 20, Z: 21, S: 20, T: 20 };
    const offer = ["X", "Y", "Z", "S", "T"];

    const arr = [...skuMap.entries()]
      .filter(([sku]) => offer.includes(sku))
      .flatMap(([sku, count]) => Array(count).fill(prices[sku]))
      .sort((a, b) => b - a);

    const groups = Math.floor(arr.length / 3);

    return groups * 45 + arr.slice(groups * 3).reduce((s, p) => s + p, 0) || 0;
  }
