export default class CheckoutSolution {
  ...
  getNewPrice(skuMap) {
    const newOfferItems = ["S", "T", "Y", "X", "Z"];
    const newExistingOfferItems = skuMap
      .keys()
      .filter((key) => newOfferItems.includes(key));
    const newTotalCount = newExistingOfferItems.reduce(
      (acc, key) => acc + skuMap.get(key),
      0
    );
    let returnValue = 0;
    if (newTotalCount >= 3) {
      returnValue += Math.floor(newTotalCount / 3) * 45;
      const times3Rest = newTotalCount % 3;
      const zItem = skuMap.get("Z");
      if (zItem) {
        returnValue += Math.floor(xItem / 3) * 17;
        skuMap.set("X", xItem % 3);
      }
      const yItem = skuMap.get("Y");
      if (yItem) {
        returnValue += Math.floor(yItem / 3) * 21;
      }
    } else {
      // if()
    }
  ...
  checkout(skus) {
    ...
    if (kItem) {
      ...
      total += times2 * 120 + times2Rest * 70;
    }
    ...
    skuMap.entries().forEach(([key, value]) => {
      if (key === "A") {
        ...
        total += 10 * value;
        // } else if (key === "S") {
        //   total += this.getNewPrice("S", value);
        // } else if (key === "T") {
        //   total += this.getNewPrice("T", value);
      } else if (key === "W") {
        total += 20 * value;
      }
      // } else if (key === "X") {
      //   total += this.getNewPrice("X", value);
      // } else if (key === "Y") {
      //   total += this.getNewPrice("Y", value);
      // } else if (key === "Z") {
      //   total += this.getNewPrice("Z", value);
      // }
    });
