export default class CheckoutSolution {
  ...
  getNewPrice(skuMap) {
    ...
    if (["S", "T", "Y"].includes(sku)) {
      return Math.floor(quantity / 3) * 45 + (quantity % 3) * 20;
    }
    if (sku === "X") {
      return Math.floor(quantity / 3) * 45 + (quantity % 3) * 17;
    }
    if (sku === "Z") return Math.floor(quantity / 3) * 45 + (quantity % 3) * 21;
  }
