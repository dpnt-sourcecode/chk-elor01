export default class CheckoutSolution {
  ...
  getNewPrice(skuMap) {
    ...
    let newTotalCount = newExistingOfferItems.reduce(
      (acc, key) => acc + skuMap.get(key),
      0
    );
    let returnValue = 0;
    if (newTotalCount >= 3) {
      const times3 = Math.floor(newTotalCount / 3);
      const times3Rest = Math.floor(newTotalCount % 3);
      returnValue += times3 * 45;
      newTotalCount -= times3 * 3;
      const xItem = skuMap.get("X");
      if (xItem) {
        returnValue += times3Rest * 17;
      }
    }
    return returnValue;
  }
