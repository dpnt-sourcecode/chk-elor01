export default class CheckoutSolution {
  ...
  getNewPrice(skuMap) {
    ...
    let newOfferMaxItem = -1;
    let newOfferMaxKey = "";
    newExistingOfferItems.forEach((item) => {
      if (skuMap.get(item) > newOfferMaxItem) {
        newOfferMaxItem = item;
        newOfferMaxKey = key;
      }
    });

    if (newExistingOfferItems.length >= 3) {
      return Math.floor(newExistingOfferItems.length / 3) * 45;
    }
