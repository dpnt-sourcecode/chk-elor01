export default class CheckoutSolution {
  ...
  checkout(skus) {
    ...
    if (vItem) {
      ...
      const times2Rest = Math.floor((vItem - times3) % 2);
      total += vItem - (times2 * 2 + times3 * 3) * 50;
    }
    ...
    if (hItem) {
      const times10 = Math.floor(value / 10);
      const times5 = Math.floor((value - times10 * 10) / 5);
      total +=
        times10 * 80 + times5 * 45 + 10 * (value - times10 * 10 + times5 * 5);
    }
